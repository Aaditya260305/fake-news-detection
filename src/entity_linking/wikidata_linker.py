...
class WikidataLinker:
    ...
    def search(self, mention: str) -> LinkedEntity:
        ...
        if data is None:
            # transient failure -- do NOT write to cache; future run retries.
            return LinkedEntity(mention=mention, qid=None, label=None, description=None)

        results = data.get("search", []) if isinstance(data, dict) else []
        if not results:
            # confirmed empty: cache so we never re-query this mention.
            self.cache.put_search(m, _NEGATIVE_SEARCH)
            return LinkedEntity(mention=mention, qid=None, label=None, description=None)
        ...
        self.cache.put_search(m, record)
    ...
    def link(self, mention: str) -> LinkedEntity:
        ent = self.search(mention)
        if ent.qid:
...
def _clean_mention(s: str) -> str:
    """Strip stray punctuation/whitespace around a mention.

    NER taggers sometimes hand us substrings like ``\"Force Protection``
    or ``Bravo:``. Trimming such cruft saves a *lot* of pointless API
    calls.
    """
    if not s:
        return ""
